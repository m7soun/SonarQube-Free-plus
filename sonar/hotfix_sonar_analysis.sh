...
if [ $TOTAL -gt 0 ]; then
    ...
    echo "Getting the latest final tag excluding release candidates..."
    LATEST_TAG=$(git tag --list 'v*.*.*' | grep -vE 'rc|hf' | sort -V | tail -n1)

    if [ -z "$LATEST_TAG" ]; then
        echo "Failed to find a final tag. Exiting..."
        exit 1
    fi

    echo "Checking out the latest final tag: $LATEST_TAG..."
    ...
    sonar-scanner \
        ...
        -Dsonar.projectVersion="$LATEST_TAG" \
...
LATEST_TAG=$(git tag --list 'v*.*.*' | grep -vE 'rc' | sort -V | tail -n1)
