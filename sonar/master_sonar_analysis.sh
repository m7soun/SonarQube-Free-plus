...
if [ $TOTAL -gt 0 ]; then
    ...
    if [ -n "$LATEST_TAG" ]; then
        ...
        echo "No valid tags found. master branch will be pushed"
        LATEST_TAG="NA"
    fi
...
sonar-scanner \
    ...
    -Dsonar.host.url="$SONAR_HOST_URL" \
    -Dsonar.token="$SONAR_TOKEN" \
    -Dsonar.projectDescription="analysis based on commit $SECOND_LAST_COMMIT_HASH" \
    -Dsonar.analysis.comment="Commit: $SECOND_LAST_COMMIT_HASH" \
    -Dsonar.qualitygate.wait=true \
    -Dsonar.scm.provider=git
